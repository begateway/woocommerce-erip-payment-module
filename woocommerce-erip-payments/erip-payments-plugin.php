...
class SPYR_ERIP_GATEWAY extends WC_Payment_Gateway {
    ...
    if ($order->payment_method != 'SPYR_ERIP_GATEWAY')
      return;
		...
		if (self::$optionsPlugin['type_sposoba_oplati'] == 'manual') {
			...
			$dataPaymentsEripSystem = $api->getInfoPaymentsWithOrderID($order->id);
	...
	*/
    ...
    sprintf('Инструкция об оплате заказа № %s', $order->order_number ), $message_body );
  	// Client email, email subject and message.
		$result = $mailer->send( $order->billing_email, sprintf( 'Инструкция об оплате заказа № %s', $order->order_number ), $message );
	}
