...
$settings = array(
  ...
  'erip_name_provider_uslugi' => array(
    'title'		=> __( 'Название поставщика услуг', 'woocommerce-begateway-erip' ),
    'type'		=> 'text',
    'desc_tip'	=> __( 'Введите ваше название поставщика услуг в системе ЕРИП', 'woocommerce-begateway-erip' ),
    'default'	=> '',
  ),
  ...
  'info_message_in_check' => array(
    ...
    'description' => __( '{{fio}}, {{order_number}}, {{name_shop}}, {{name_sevice_provider}}', 'woocommerce-begateway-erip' ),
  ),
  ...
  'description_configuration_manual_mode' => array(
    ...
    'type'		=> 'textarea',
    'desc_tip'	=> __( 'Введите текст-подтверждение заказа для ручного режима', 'woocommerce-begateway-erip' ),
    'default'	=> __( 'Спасибо за ваш заказ. Наш менеджер свяжется с вами для уточнения заказа и сообщит номер заказа и инструкции как его оплатить в системе ЕРИП.', 'woocommerce-begateway-erip' ),
    'css'		=> 'max-width:80%;'
  ),
  ...
  'description_configuration_auto_mode' => array(
    ...
    'type'		=> 'textarea',
    'desc_tip'	=> __( 'Введите текст-подтверждение заказа для автоматического режима', 'woocommerce-begateway-erip' ),
    'default'	=> __( 'Спасибо за заказ.
Для проведения платежа через ЕРИП необходимо:

1.​ Выбрать пункт ЕРИП
2.​ Выбрать последовательно вкладки: {{instruction_erip}}
3.​ Ввести номер заказа {{order_number}}
4.​ Проверить корректность информации
5.​ Совершить платеж.', 'woocommerce-begateway-erip' ),
    'css'		=> 'max-width:80%;',
    'description' => __( '{{fio}}, {{order_number}}, {{name_shop}}, {{name_sevice_provider}}, {{instruction_erip}}', 'woocommerce-begateway-erip' ),
  ),
...
В каталоге сиcтемы ЕРИП услуги {{name_sevice_provider}} находятся в разделе:

ЕРИП -> {{instruction_erip}}
