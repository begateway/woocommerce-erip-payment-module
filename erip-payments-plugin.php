  	...
  	public function handle_callback() {
		...
		if ($order->id != 0) {
			...
			if ($post_array->transaction->status == 'successful') {
				...
				echo 'Данные успешно получены! Заказ поставлен на выполнение.';
			} else {
				echo 'Данные успешно получены! Статус заказа не изменён.';
			}			
		} else {
			echo 'Ошибка! Заказ не найден в системе.';
		}
/*
...
class SPYR_ERIP_GATEWAY extends WC_Payment_Gateway {
	static private $optionsPlugin = null;
	...
	public function create_invoice_with_erip(&$order_sybmol_link) {
		...
		$arrayDataInvoice = [
 			"request" => [
				...
				"notification_url" => home_url('/').'wc-api/'.strtolower('WC_mygateway'),
				"customer" => [
	 			...
	 			"payment_method" => [
					"type" => "erip",
					"account_number" => $this->get_option( 'erip_id_magazin' ),
					"service_no" => $this->get_option( 'erip_kod_uslugi' ),
					"service_info" => [
						"Уважаемый клиент,",
						"Вы оплачиваете заказ ".$order_sybmol_link->get_order_number()
					],
					"receipt" => [
		...
		if (!$response) {
			wc_add_notice( curl_error($ch), 'error' );
		}
		curl_close($ch);
	...
	*/
	...
	public function process_payment( $order_id ) {
		...
		if ($this->get_option( 'type_sposoba_oplati' ) == 'manual') {
			...
			// Mark as on-hold (we're awaiting the cheque)
			//$order->update_status('Pending payment', __( 'Ожидается оплата заказа', 'woocommerce-erip-payments' ));
